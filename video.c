...
static SDL_Window *  cpu_window;
static SDL_Renderer *cpu_renderer;
static SDL_Texture * cpu_sdlTexture;
...
static uint8_t cpu_line_status[SCAN_WIDTH];
static uint8_t cpu_line_pc[SCAN_WIDTH];
static uint8_t cpu_framebuffer[SCAN_WIDTH * SCAN_HEIGHT * 4];
...
video_init(int window_scale, char *quality)
{
	...
	SDL_CreateWindowAndRenderer(SCAN_WIDTH * window_scale, SCAN_HEIGHT * window_scale, SDL_WINDOW_ALLOW_HIGHDPI, &cpu_window, &cpu_renderer);
#ifndef __MORPHOS__
	...
	SDL_SetWindowTitle(cpu_window, "X16 execution visualization");
	{
		...
		SDL_SetWindowPosition(cpu_window, x + SCREEN_WIDTH * window_scale + right, y);
	}

	cpu_sdlTexture = SDL_CreateTexture(cpu_renderer, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, SCAN_WIDTH, SCAN_HEIGHT);
...
{
	// Look up all color indices.
	uint32_t *framebuffer4_begin = ((uint32_t *)cpu_framebuffer) + (scan_pos_y * SCAN_WIDTH);
	{
		uint32_t *framebuffer4 = framebuffer4_begin;
		for (uint16_t x = 0; x < SCAN_WIDTH; x++) {
			uint32_t pc_coloration = (uint32_t)cpu_line_pc[x] | ((uint32_t)cpu_line_pc[x] << 8) | ((uint32_t)cpu_line_pc[x] << 16);
			if (cpu_line_status[x] & FLAG_BREAK) {
				*framebuffer4++ = (pc_coloration & 0x00ff00ff);
			} else if (cpu_line_status[x] & FLAG_INTERRUPT) {
				*framebuffer4++ = (pc_coloration & 0x00ffff00);
			} else {
				*framebuffer4++ = (pc_coloration & 0x0000ffff);
			}
		}
	}

	{
		...
		uint8_t out_mode      = reg_composer[0] & 3;
		int     front_porch_x = (out_mode & 2) ? NTSC_FRONT_PORCH_X : VGA_FRONT_PORCH_X;
		int     front_porch_y = (out_mode & 2) ? NTSC_FRONT_PORCH_Y : VGA_FRONT_PORCH_Y;

		if ((scan_pos_y < front_porch_y) || (scan_pos_y > (front_porch_y + SCREEN_HEIGHT))) {
			for (uint16_t x = 0; x < SCAN_WIDTH; x++) {
				//// Divide RGB elements by 4.
				//*framebuffer4 &= 0x00fcfcfc;
				//*framebuffer4 >>= 2;

				// Subtract 1/4th brightness
				*framebuffer4 -= ((*framebuffer4 & 0x00fcfcfc) >> 2);

				framebuffer4++;
			}
		} else {
			for (uint16_t x = 0; x < SCAN_WIDTH; x++) {
				if ((x < front_porch_x) || (x > (front_porch_x + SCREEN_WIDTH))) {
					//// Divide RGB elements by 4.
					//*framebuffer4 &= 0x00fcfcfc;
					//*framebuffer4 >>= 2;
...
video_step(float mhz)
{
	...
	for (int i = scan_pos_x; (i < scan_pos_x + advance) && (i < SCAN_WIDTH); ++i) {
		cpu_line_status[i] = get_status6502();
		cpu_line_pc[i]     = get_pc6502() >> 8;
	}
	...
	if (scan_pos_x > SCAN_WIDTH) {
		...
		for (int i = 0; (i < scan_pos_x) && (i < SCAN_WIDTH); ++i) {
			cpu_line_status[i] = get_status6502();
			cpu_line_pc[i]     = get_pc6502() >> 8;
		}
...
video_update()
{
	...
	SDL_UpdateTexture(cpu_sdlTexture, NULL, cpu_framebuffer, SCAN_WIDTH * 4);
...
video_end()
{
	...
	SDL_DestroyRenderer(cpu_renderer);
	SDL_DestroyWindow(cpu_window);
}
...
uint8_t video_read(uint8_t reg, bool debugOn) {
	switch (reg & 0x1F) {
		...
		case 0x06: return ((irq_line & 0x0100) >> 1) | (ien & 0xF);
